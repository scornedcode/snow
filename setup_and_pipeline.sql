...
-- Time Travel Demo
CREATE OR REPLACE TABLE AIRLINE_DWH.CORE.FLIGHTS_HISTORY_DEMO CLONE AIRLINE_DWH.CORE.FLIGHTS;
-- DML 1: Update data
...
-- Time Travel Queries (DDL/Selection)
-- 1. Select data from 5 minutes ago
...
-- Security: Row Level Security using Access Policy (Modern approach)
USE SCHEMA AIRLINE_DWH.CORE;
...
CREATE OR REPLACE ROW ACCESS POLICY COUNTRY_ACCESS_POLICY AS (country_val VARCHAR) RETURNS BOOLEAN ->
  CASE
    ...
    WHEN CURRENT_ROLE() = 'ANALYST_CN' AND country_val = 'China' THEN TRUE
    ELSE FALSE
  END;

-- Apply Policy to Fact Table (or create a view if preferred)
CREATE OR REPLACE VIEW AIRLINE_DWH.MARTS.SECURE_FLIGHTS_VIEW AS
SELECT f.*, a.COUNTRY_NAME
FROM AIRLINE_DWH.CORE.FLIGHTS f
JOIN AIRLINE_DWH.CORE.AIRPORTS a ON f.AIRPORT_ID = a.AIRPORT_ID;

-- Attach the policy to the view column
